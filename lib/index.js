...
handlePosition = 'outside', disableScrollIfPossible = true, avoidKeyboardLikeIOS = react_native_1.Platform.select({
    ...
    android: false,
    default: true,
}), keyboardAvoidingBehavior = 'padding', keyboardAvoidingOffset, panGestureEnabled = true, panGestureComponentEnabled = false, tapGestureEnabled = true, closeOnOverlayTap = true, closeSnapPointStraightEnabled = true, disablePanGestureForChildren = false, 
// Animations
openAnimationConfig = {
...
onOpen, onOpened, onClose, onClosed, onBackButtonPress, onPositionChange, onOverlayPress, onLayout, }, ref) => {
    ...
    const handleAnimateOpen = (alwaysOpenValue, dest = 'default') => {
        ...
        console.log(!!spring, translateY, spring ? get_spring_config_1.getSpringConfig(spring) : 'no spring');
    ...
    const renderContent = () => {
        ...
        // const options = opts;
        if (flatListProps) {
            return React.createElement(react_native_1.Animated.FlatList, Object.assign({}, flatListProps, options));
        }
        if (sectionListProps) {
            return React.createElement(react_native_1.Animated.SectionList, Object.assign({}, sectionListProps, options));
        }
        if (customRenderer) {
            return React.cloneElement(customRenderer, Object.assign({}, options));
        }
        return (React.createElement(react_native_1.Animated.ScrollView, Object.assign({}, scrollViewProps, options), children));
    };
    const renderChildren = () => {
        const style = adjustToContentHeight ? styles_1.default.content__adjustHeight : styles_1.default.content__container;
        if (devices_1.isIos && disablePanGestureForChildren) {
            return (React.createElement(react_native_1.Animated.View, { style: [style, childrenStyle] }, renderContent()));
        }
        return (React.createElement(react_native_gesture_handler_1.PanGestureHandler, { ref: panGestureChildrenRef, enabled: panGestureEnabled && !disablePanGestureForChildren, simultaneousHandlers: [nativeViewChildrenRef, tapGestureModalizeRef], shouldCancelWhenOutside: false, onGestureEvent: handleGestureEvent, minDist: ACTIVATED, activeOffsetY: ACTIVATED, activeOffsetX: ACTIVATED, onHandlerStateChange: handleChildren },
            React.createElement(react_native_1.Animated.View, { style: [style, childrenStyle] },
                React.createElement(react_native_gesture_handler_1.NativeViewGestureHandler, { ref: nativeViewChildrenRef, waitFor: tapGestureModalizeRef, simultaneousHandlers: panGestureChildrenRef, enabled: panGestureEnabled && !disablePanGestureForChildren }, renderContent()))));
    };
    const renderOverlay = () => {
        const pointerEvents = alwaysOpen && (modalPosition === 'initial' || !modalPosition) ? 'box-none' : 'auto';
        if (devices_1.isIos && disablePanGestureForChildren) {
            return (React.createElement(react_native_1.Animated.View, { style: styles_1.default.overlay, pointerEvents: pointerEvents }, showContent && (React.createElement(react_native_gesture_handler_1.TapGestureHandler, { ref: tapGestureOverlayRef, enabled: closeOnOverlayTap !== undefined ? closeOnOverlayTap : panGestureEnabled, onHandlerStateChange: handleOverlay },
    ...
    const renderModalize = (React.createElement(react_native_1.View, { style: [styles_1.default.modalize, rootStyle], pointerEvents: alwaysOpen || !withOverlay ? 'box-none' : 'auto' },
        React.createElement(react_native_gesture_handler_1.TapGestureHandler, { ref: tapGestureModalizeRef, maxDurationMs: tapGestureEnabled ? 100000 : 50, maxDeltaY: lastSnap, enabled: panGestureEnabled && !disablePanGestureForChildren },
